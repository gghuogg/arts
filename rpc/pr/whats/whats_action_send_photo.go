...
func init() {
	...
	im.RegisterAction(protobuf.Action_SEND_PHOTO, &whatsSendPhotoMsg{})
}

type whatsSendPhotoMsg struct{}

func (t *whatsSendPhotoMsg) Handler(l log.Logger, p *rpc.ProxyServer, in *protobuf.RequestMessage) (result *protobuf.ResponseMessage, err error) {
	details := in.GetSendImageFileDetail().GetDetails()
	if len(details) > 0 {
		for _, detail := range details {
			for k, v := range detail.SendData {
				addMember := p.PrGetSyncMap(rpc.WS_SEND_PHOTO)
