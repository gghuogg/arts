...
func TestSendGif(t *testing.T) {
	...
	// 构建文件路径
	filePath := filepath.Join(currentDir, "testFile", "biu_biu_biu.gif")
	...
	gifList := make([]*telegram.SendFileDetail, 0)
	gifTest1 := &telegram.SendFileDetail{
		Sender:   accountNum,
		Receiver: strconv.FormatUint(8618818877128, 10),
		FileType: []telegram.SendFileType{
			{
				FileType:  telegram.TG_FILE_TYPE_GIF,
				SendType:  telegram.TG_SEND_TYPE_BYTE,
				Path:      "cmd/arthtoolTG/testFile/biu_biu_biu.gif",
				Name:      "biu_biu_biu.gif",
				FileBytes: b3,
			},
		},
	}
	gifList = append(gifList, gifTest1)
	...
	//发送gif
	//time.Sleep(5 * time.Second)
	req7 := sendGifFile(accountNum, gifList)
...
func sendGifFile(accountNum uint64, files []*telegram.SendFileDetail) *protobuf.RequestMessage {
