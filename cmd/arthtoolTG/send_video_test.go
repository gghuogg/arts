...
func TestSendVideo(t *testing.T) {
	...
	// 构建文件路径
	filePath := filepath.Join(currentDir, "testFile", "kk.mp4")
	...
	VideoList := make([]*telegram.SendFileDetail, 0)
	videoTest1 := &telegram.SendFileDetail{
		Sender:   accountNum,
		Receiver: strconv.FormatUint(8618818877128, 10),
		FileType: []telegram.SendFileType{
			{
				FileType:  telegram.TG_FILE_TYPE_VIDEO,
				SendType:  telegram.TG_SEND_TYPE_BYTE,
				Path:      "cmd/arthtoolTG/testFile/kk.mp4",
				Name:      "kk.mp4",
				FileBytes: b3,
			},
		},
	}
	VideoList = append(VideoList, videoTest1)
	...
	//发送视频
	//time.Sleep(5 * time.Second)
	req7 := sendVideoFile(accountNum, VideoList)
...
func sendVideoFile(accountNum uint64, files []*telegram.SendFileDetail) *protobuf.RequestMessage {
