...
func TestSendOgg(t *testing.T) {
	...
	// 构建文件路径
	filePath := filepath.Join(currentDir, "testFile", "weiwei.ogg")
	...
	//音频
	oggList := make([]*telegram.SendFileDetail, 0)
	oggTest1 := &telegram.SendFileDetail{
		Sender:   accountNum,
		Receiver: strconv.FormatUint(8618818877128, 10),
		FileType: []telegram.SendFileType{
			{
				FileType:  telegram.TG_FILE_TYPE_OGG,
				SendType:  telegram.TG_SEND_TYPE_BYTE,
				Path:      "cmd/arthtoolTG/testFile/weiwei.ogg",
				Name:      "weiwei.ogg",
				FileBytes: b3,
			},
		},
	}
	oggList = append(oggList, oggTest1)
	...
	//time.Sleep(5 * time.Second)
	//音频
	req7 := sendOggFile(accountNum, oggList)
...
func sendOggFile(accountNum uint64, files []*telegram.SendFileDetail) *protobuf.RequestMessage {
