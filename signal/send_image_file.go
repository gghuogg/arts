...
func (a *Account) sendImageFile(detail SendImageFileDetail) {
	...
	for _, f := range detail.FileType {
		...
		if len(f.ImageBytes) > FILE_MAX_SIZE {
			level.Error(a.logger).Log("msg", "file size over 2G")
			return
		}
		b := f.ImageBytes

		encryptedImageData, fileSha256, fileEncSha256, err := EncryptMediaBytes(b, mediaKey, []byte(labelImageKeys))
		if err != nil {
		...
		req, err := http.NewRequest(http.MethodPost, fmt.Sprintf("https://%s/optimistic/image/%s", hostname, filename), bytes.NewReader(encryptedImageData))
	...
	a.SendImageFileMessage(detail, fileKeyInfoList)

}

func (a *Account) SendImageFileMessage(detail SendImageFileDetail, fileInfolist []*FileKeyInfo) {
	//a.signal.mutex.Lock()
	//
